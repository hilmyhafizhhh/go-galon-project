...
.ca-field:last-child{border-bottom:none}
.dark .ca-field{border-bottom-color:rgba(255,255,255,.05)}
.ca-field__icon{
    width:32px;height:32px;flex-shrink:0;
    background:var(--blue-50);border-radius:var(--radius-sm);
    display:flex;align-items:center;justify-content:center;
    color:var(--blue-600);margin-top:2px;
    border:1px solid rgba(37,99,235,.12);
}
.dark .ca-field__icon{background:rgba(37,99,235,.12);border-color:rgba(37,99,235,.22);color:#60a5fa}
.ca-field__body{flex:1;min-width:0}
.ca-field__label{
...
/* ── Boxed inputs — same as profile page ── */
.ca-field__input,
.ca-field__textarea{
...
.ca-field__textarea::placeholder{color:var(--slate-300)}
.dark .ca-field__input::placeholder,
.dark .ca-field__textarea::placeholder{color:#475569}
...
.ca-field__error{
    ...
    font-size:.72rem;color:#e11d48;font-weight:600;
}
...
.ca-map-trigger:hover{background:var(--blue-50)}
.dark .ca-map-trigger{border-bottom-color:rgba(255,255,255,.05)}
.dark .ca-map-trigger:hover{background:rgba(37,99,235,.07)}
...
.dark .ca-map-trigger__ico{background:rgba(37,99,235,.12);border-color:rgba(37,99,235,.22);color:#60a5fa}
.ca-map-trigger:hover .ca-map-trigger__ico{background:var(--blue-100);transform:scale(1.05)}

.ca-map-trigger__body{flex:1;min-width:0}
.ca-map-trigger__title{
    font-size:.88rem;font-weight:700;
    color:var(--slate-800);line-height:1.3;
}
.dark .ca-map-trigger__title{color:#f1f5f9}
.ca-map-trigger__sub{
    font-size:.73rem;color:var(--slate-400);
...
.dark .ca-map-trigger__badge{background:rgba(37,99,235,.12);color:#60a5fa;border-color:rgba(37,99,235,.3)}
...
.ca-map-expand.open{display:flex}
@keyframes caFadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}

#ca-map{width:100%;height:240px;z-index:1;cursor:grab}
#ca-map:active{cursor:grabbing}
...
.ca-map-hint{
    ...
    font-size:.73rem;color:var(--slate-400);font-weight:500;
}
.dark .ca-map-hint{background:rgba(37,99,235,.07);border-top-color:rgba(37,99,235,.15)}
.ca-map-hint svg{flex-shrink:0;color:var(--blue-600)}
.dark .ca-map-hint svg{color:#60a5fa}
...
.ca-map-result.show{display:flex}
.dark .ca-map-result{background:rgba(37,99,235,.09);border-top-color:rgba(37,99,235,.18)}
.ca-map-result__chip{
    display:inline-flex;align-items:center;gap:5px;
    font-size:.65rem;font-weight:700;letter-spacing:.5px;text-transform:uppercase;
    color:var(--blue-600);
}
.dark .ca-map-result__chip{color:#60a5fa}
.ca-map-result__addr{
    font-size:.8rem;color:var(--slate-700);line-height:1.5;font-weight:500;
}
.dark .ca-map-result__addr{color:var(--slate-300)}
...
.ca-map-confirm:hover{box-shadow:0 6px 20px rgba(37,99,235,.42);transform:translateY(-1px)}
.ca-map-confirm.show{display:flex}

/* ─ Detected chip on trigger ─────── */
.ca-map-trigger__detected{
    display:none;
    font-size:.73rem;color:var(--blue-600);margin-top:3px;font-weight:600;
    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
}
.dark .ca-map-trigger__detected{color:#60a5fa}
.ca-map-trigger__detected.show{display:block}
...
.ca-toggle-row__left{display:flex;align-items:center;gap:10px}
.ca-toggle-row__ico{
...
.dark .ca-toggle-row__ico{background:rgba(37,99,235,.12);border-color:rgba(37,99,235,.22);color:#60a5fa}
.ca-toggle-row__text .ca-toggle-row__title{font-size:.88rem;font-weight:700;color:var(--slate-800)}
.dark .ca-toggle-row__text .ca-toggle-row__title{color:#f1f5f9}
.ca-toggle-row__text .ca-toggle-row__sub{font-size:.72rem;color:var(--slate-400);margin-top:1px}

.ca-switch{position:relative;width:44px;height:26px;flex-shrink:0}
.ca-switch input{opacity:0;width:0;height:0;position:absolute}
.ca-switch__track{
    position:absolute;inset:0;
    background:var(--slate-200);border-radius:13px;
    transition:background .22s;cursor:pointer;
}
.dark .ca-switch__track{background:var(--slate-300)}
.ca-switch__thumb{
    position:absolute;
    top:3px;left:3px;
...
.ca-switch input:checked~.ca-switch__track{background:var(--blue-600)}
.ca-switch input:checked~.ca-switch__thumb{transform:translateX(18px)}
...
.ca-info-strip{
    ...
    background:var(--blue-50);
    border-top:1px solid rgba(37,99,235,.08);
    font-size:.72rem;color:var(--slate-500);line-height:1.5;
}
.dark .ca-info-strip{background:rgba(37,99,235,.08);border-top-color:rgba(37,99,235,.15)}
.ca-info-strip svg{flex-shrink:0;color:var(--blue-600);margin-top:1px}
.dark .ca-info-strip svg{color:#60a5fa}
...
.ca-footer__inner{max-width:720px;margin:0 auto}
.ca-footer__btn{
...
.ca-footer__btn:hover{box-shadow:0 6px 20px rgba(37,99,235,.45);transform:translateY(-1px)}
.ca-footer__btn:disabled{opacity:.45;cursor:not-allowed;transform:none}
...
.dark .leaflet-control-zoom a{background:#1e293b!important;color:#60a5fa!important}
</style>
...
<div class="ca-header">
    <div class="ca-header__inner">
        ...
        <div class="ca-header__titles">
            <div class="ca-header__title">Tambah Alamat Baru</div>
            <div class="ca-header__sub">Isi detail lokasi pengiriman kamu</div>
        </div>
    ...
    <form action="{{ route('customer.address.store') }}" method="POST" id="caForm">
        ...
        <div class="ca-card ca-reveal" data-delay="40">
            ...
            <div class="ca-field" style="padding:0;border-bottom:1px solid rgba(37,99,235,.06);display:block">
                <div class="ca-map-trigger" id="caMapTrigger" role="button" tabindex="0"
                    aria-expanded="false" aria-label="Gunakan lokasi saat ini">
                    ...
                    <div class="ca-map-trigger__body" style="min-width:0">
                        <div style="display:flex;align-items:center;gap:7px">
                            <span class="ca-map-trigger__title">Gunakan Lokasi Saat Ini</span>
                    ...
                    <svg id="caMapChevron" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2"
                        stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"
            ...
            <div class="ca-field" style="border-bottom:none">
                ...
                <div class="ca-field__body">
                    ...
                    <textarea id="ca_address" name="address" class="ca-field__textarea" rows="3"
                        placeholder="Nama jalan, nomor, RT/RW, kelurahan, patokan…">{{ old('address') }}</textarea>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    trigger.addEventListener('click', () => {
        expand.classList.contains('open') ? closeMap() : openMap();
    });
    trigger.addEventListener('keydown', e => { if(e.key==='Enter'||e.key===' ') { e.preventDefault(); trigger.click(); } });

    function initMap() {
        mapReady = true;
        const defLat = -6.2088, defLng = 106.8456;

        map = L.map('ca-map', {
            center:[defLat, defLng], zoom:16,
        ...
        const pinSvg = `<div style="width:36px;height:36px;position:relative;transform:translate(-50%,-100%)">
            ...
            <div style="position:absolute;bottom:-2px;left:50%;transform:translateX(-50%);
                width:6px;height:6px;background:rgba(37,99,235,.3);border-radius:50%;
                animation:caPinShadow 1.2s ease-in-out infinite"></div>
        </div>`;
